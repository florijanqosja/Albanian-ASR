...
export default function Pricing() {
  ...
  useEffect(() => {
    async function fetchSummaryInfo() {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_DOMAIN_DEV}dataset_insight_info`);
    ...
    <Box className={classes.root}>
      <Grid container justify="center">
        <Grid item xs={14} sm={8} md={7}>
          ...
          <Typography variant="h2" className={classes.title}>OUR DATASET SO FAR dev</Typography>
          {summaryInfo && (
