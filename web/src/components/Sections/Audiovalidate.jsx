...
export default function AudioValidater() {
  ...
  const fetchAudioData = async () => {
    try {
      const response = await axios.get(`${process.env.DEV_REACT_APP_API_DOMAIN}audio/to_validate`);
      const { Sp_PATH, Sp_ID } = response.data;
      const audioURL = `${process.env.DEV_REACT_APP_FILE_ACCESS_DOMAIN}${Sp_PATH}`;
  ...
  const deleteAudio = async () => {
    try {
      await axios.delete(`${process.env.DEV_REACT_APP_API_DOMAIN}audio`, {
        data: {
          Sp_ID: audioID,
        },
      });
  ...
  const handleSubmit = async () => {
    try {
      await axios.put(`${process.env.DEV_REACT_APP_API_DOMAIN}audio/validate`, {
        Sp_ID: audioID,
        Sp_LABEL: labelValue,
      });
