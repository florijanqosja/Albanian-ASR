...
export default function MainSection() {
  ...
  const fetchAudioData = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_DOMAIN_LOCAL}audio/to_label`);
      if (response.data && response.data.data) {
        const { path, id } = response.data.data;
        ...
        console.log("No audio data available to label.");
      }
    } catch (error) {
  ...
  const deleteAudio = async () => {
    ...
    try {
      ...
      if (wavesurferRef.current) {
          ...
          // wavesurferRef.current.empty(); // empty() is not in v7, load('') or similar might work but usually just stop is enough or load new
      }
      setLabelValue(""); 
