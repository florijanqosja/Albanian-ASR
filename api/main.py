...
def predict_word(f_path):
    ...
    frame_step = 160
    fft_length = 384
    # f_path = "/code/splices/flori/321_dite_lufte_ne_ukraine_xhelilaj_rezistence_mbreslenese_nje_samit_paqeje_mund_ti_jape_fund_189.350-190.550.wav"
...
async def create_upload_file(file_wav: UploadFile = File(...)):
    dirs_to_check = [
        "/code/totranscribe",
        "/code/totranscribe/original",
        "/code/totranscribe/resampled"
    ]
    for dir_path in dirs_to_check:
        os.makedirs(dir_path, exist_ok=True)
    now = datetime.now()
    file_name = now.strftime("%d_%m_%Y_%H_%M_%S")
    path_to_save_original = f"/code/totranscribe/original/{file_name}.wav"
    ...
    os.system(f"ffmpeg -i {path_to_save_original} -ac 1 -ar 16000 {path_to_resampled}")
